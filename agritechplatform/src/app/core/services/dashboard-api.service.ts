...
export class DashboardApiService {
  ...
  private fetchBlockTimeseries(blockId: string): Observable<DashboardTimeseriesPoint[]> {
    return this.http.get<unknown>(`${this.baseUrl}/api/block/${blockId}/timeseries`).pipe(
      map(payload => timeseriesSchema.parse(payload)
        .map((point: SatelliteTimeseriesContract) => this.mapTimeseriesPoint(point))
        .sort((left: DashboardTimeseriesPoint, right: DashboardTimeseriesPoint) => {
  ...
  private mapToDashboardInsights(
    ...
    return {
      ...
      confidence: this.deriveConfidence(response),
      dataAgeDays: this.calculateDataAgeDays(response.composite_date_to),
  ...
  private buildActionItems(response: BlockInsightsContract): DashboardActionItem[] {
    if (response.alerts.length) {
      return response.alerts.map((alert: BlockInsightsContract['alerts'][number], index: number) => ({
        priority: index + 1,
    ...
    if (response.limitations.length) {
      return response.limitations.map((limitation: string, index: number) => ({
        priority: index + 1,
  ...
  private deriveConfidence(response: BlockInsightsContract): string {
    if (response.data_quality === 'no_data') {
      return 'low';
    }
    if (response.pixel_count < 20) {
      return 'low';
    }
    if ((response.cloud_cover_pct ?? 0) > 50 || response.data_quality === 'degraded') {
      return 'medium';
    }
    return 'high';
  }

  private calculateDataAgeDays(compositeDateTo: string | null): number {
    if (!compositeDateTo) {
      return 0;
    }

    const compositeDate = this.parseDateValue(compositeDateTo);
    if (Number.isNaN(compositeDate.getTime())) {
      return 0;
    }

    const now = new Date();
    const diffMs = now.getTime() - compositeDate.getTime();
